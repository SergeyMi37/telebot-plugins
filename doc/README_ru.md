...
Реализованы модули в плагинах:
- /data - Получение информации по телефонам и Организациям (Нужен ключ пользователя https://dadata.ru/api/)
- /chinese - Для тех кто учит китайский язык. Поиск этимологии иероглифов и перевод на английский (Используется библиотека playwrite и сайт https://www.archchinese.com/ )

# Проект чат-бота с Django админкой и с плагинами и настройкой приложения через Dynaconf

В настоящее время поддерживаются плагины:
- /chat - Собеседник с моделью от ГигаЧат (нужен ключ разработчика)
- /chat_list - Чаты с моделями из Ollama (нужна установленная в локальной сети платформа Ollama)
- /iris - Мониторинг серверов IRIS от фирмы Интерсистемс 
- /gitlab - Репортер по задачам из проектов GitLab (нужен токен для чтения из проекта)
- /weather - Прогноз погоды /weather_list - список /weather_Moscow в Москве на день и 7 дней 
...
- /wiki - Сервис получения статей из Википедии. Например, /wiki_Snow или /wiki_ - ввести слово
- /news - Сервис агрегации rss новостей. /news_list - список лент
- /inet - Сервис поиска в интернет /inet_ddg_ - пока только в поисковой системе DuckDuckGo
- /admin - Модуль администрирования в группах
    удаление системной информации о входе и выходе из групп
    удаления пользователя из группы за написание запрещенных слов
    получения отчетов по пользователям и изменениям в формате CSV
- /code - Модуль получения информации из справочников кодов регионов РФ и поиск страны по началу штихкода
